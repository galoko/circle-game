...
function stepAI(dt: number) {
    ...
    if (teamA === 0 || teamB === 0) {
        alert(
            `Game over! ${teamB === 0 ? "Team 6" : "Team 7"} wins! Battle duration: ${Math.ceil(
                currentTime
            )} seconds.`
        )
        won = true
...
;(window as any).userActivity = 0.1

function stepMessages() {
    if (currentTime - lastMessageTime > (window as any).userActivity) {
...
function fitCameraToPlayers(
    ...
    // x' =  x*cos + y*sin
    // y' = -x*sin + y*cos
    function evalTheta(theta: number) {
...
function tick(time: number) {
    ...
    cameraRotation = Math.atan2(
        boundsWidth - screenAspect * boundsHeight,
        screenAspect * boundsWidth - boundsHeight
    )
    ...
    cameraScale = Math.min(
        ctx.canvas.width / (rotatedWidth * margin),
        ctx.canvas.height / (rotatedHeight * margin)
    )
